...
const FilterSidebar = ({ uniqueFilters }) => {
  // State to track which filter group is currently open
  const [activeSection, setActiveSection] = useState("Color"); // Default 'Color' open, or use null for all closed
        ...
        {uniqueFilters.map((filterGroup) => {
          ...
          return (
            <details key={filterGroup.id} className="group" open={isOpen}>
              {/* We use onClick with preventDefault to stop the browser's native 
                  toggle behavior, giving React full control over the open state.
              */}
              <summary
                onClick={(e) => {
                  e.preventDefault();
              ...
              >
                ...
                {/* Rotate chevron based on the 'isOpen' variable */}
                <ChevronDown
                  size={18}
